...
export default class CreditCard extends React.Component {
  ...
  onSelectAccount = (name, number) => {
    this.setState({
      chosenType: { name: name, number: number }
    })
  }

  handleChooseAccount = () => {
    const { navigation } = this.props;
    navigation.navigate('CreditCardType', { onSelectAccount: this.onSelectAccount })
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.subContainer}>
          <Text style={styles.textLabel}>CreditCard Type</Text>
          <TouchableHighlight onPress={() => this.handleChooseAccount()} underlayColor="rgb(230, 230, 230)" style={styles.buttonDestinationAccount}>
            <TextInput
              ...
              value={this.state.chosenType.name}
            />
          </TouchableHighlight>
        </View>
        <View style={styles.subContainer}>
          <Text style={styles.textLabel}>CreditCard Number</Text>
          <TextInput placeholder="E.g. 5555666677778888" style={styles.textInput} keyboardType="number-pad"></TextInput>
        </View>
        <View style={styles.subContainer}>
